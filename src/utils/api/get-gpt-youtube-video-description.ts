export async function getGptYoutubeVideoDescription(
  ...
  const result = await window.fetch(
    new Request("/api/chat-gpt-3/get-answer", {
      ...
      body: JSON.stringify({
        message: `Вы выступаете в роли SEO (англ. Search Engine Optimization) специалиста. Выша задача придумать хорошее ёмкое описание для Youtube видео с названием "${question}".
